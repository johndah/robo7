function detector = acfTrainCustom( varargin )
...
% candidates that are used for classification. For example, 0.67'
...
for stage = 0:numel(opts.nWeak)-1
  ...
  if stage==0
  fprintf([repmat('-',[1 75]) '\n']);
  fprintf(['modelDs: ' num2str(opts.modelDs) '   modelDsPad: ' num2str(opts.modelDsPad) '\n']);
  fprintf(['object: ' opts.pLoad.lbls '\n']);
  fprintf(['squarify: ' num2str(cell2mat(opts.pLoad.squarify)) '\n']);
  fprintf(['Color enable: ' num2str(opts.pPyramid.pChns.pColor.enabled), '   colorspace: ' opts.pPyramid.pChns.pColor.colorSpace '\n']);
  fprintf(['Gradient Magnitude enable: ' num2str(opts.pPyramid.pChns.pGradMag.enabled) '\n']);
  fprintf(['Histogram Orients enable: ' num2str(opts.pPyramid.pChns.pGradHist.enabled) '   nOrients: ' num2str(opts.pPyramid.pChns.pGradHist.nOrients) '\n']);
  end
...
% Input     positive: 1 for positive images, 0 for negative
% Output    Is contains miror images of IsOrig
...
if( exist(crDir,'dir') && stage==0 ) % && stage==0 
  ...
  fs={opts.posImgDir,opts.posGtDir}; fs=bbGt('getFiles',fs);
  if(~hasGt)
      ffs={opts.negImgDir};
      ffs=bbGt('getFiles',ffs);
      ffs=[ffs;repmat({[]},1,length(ffs))];
      fs=[fs,ffs];
  end
  ...
  while( i<nImg && k<n )
    ...
    for j=1:batch, ij=i+j;
      ...
      % downsampling original images
      I = imresize(I, opts.pLoad.rescale);
