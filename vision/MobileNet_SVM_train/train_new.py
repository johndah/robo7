...
def run_training():
    ...
    iterator = input_fn(train=True, batch_size=args.batch_size, num_epochs=args.epoch)
    image_batch, label_batch = iterator.get_next()

    logits, pred = mobilenetv2(image_batch, num_classes=args.num_classes, is_train=True)
    ...
    for _step in range(step + 1, max_steps + 1):
        ...
        if _step % 10 == 0:
            feed_dict = {global_step: _step}
            _summ, _loss, _acc, _pred = sess.run([summary_op, total_loss, acc, correct_pred],
                                          feed_dict=feed_dict)
